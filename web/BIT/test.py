...
predictor = Predictor("./BIT/static_model", use_gpu=False)
...
"""
# 下面的语句传入两幅输入影像的路径
res = predictor.predict(("./saved_files/bit/A.png", "./saved_files/bit/B.png"))
print(res)
...
plt.subplot(131)
plt.imshow(Image.open("./saved_files/bit/A.png"))
plt.gca().set_axis_off()
plt.title("Image1")
plt.subplot(132)
plt.imshow(Image.open("./saved_files/bit/B.png"))
plt.gca().set_axis_off()
plt.title("Image2")
plt.subplot(133)
...
plt.gca().set_axis_off()
plt.title("Pred")
cv2.imwrite("./saved_imgs/bit/test.png",(cm_1024x1024*255).astype('uint8'))
plt.savefig('./saved_imgs/bit/bit_res.png')
# plt.subplot(144)
