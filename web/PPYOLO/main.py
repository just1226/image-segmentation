...
predictor = Predictor("./PPYOLO/PPYOLO_model", use_gpu=False)
...
"""
# 下面的语句传入两幅输入影像的路径
res = predictor.predict(("./saved_files/odt/img.png"))
result = []
for r in res:
        score_thesh=r["score"]
        if score_thesh>0.5:
...
imgpath="./saved_files/odt/img.png"
#res=predict(imgpath)
#print(res)
...
for r in result:
    print(r)
    b=r["bbox"]
    cv2.rectangle(img,(int(b[0]),int(b[1])),(int(b[0])+int(b[2]),int(b[1])+int(b[3])),(0,255,0),2)
    cv2.putText(img,r["category"]+":"+str(round(r["score"],2)),(int(b[0])+2,int(b[1])+2),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,0,0),1)

# cv2.imshow("result",img)
cv2.imwrite("./saved_imgs/odt/odt_res.png",img)
cv2.waitKey(0)
