...
predictor = Predictor('./classify/export', use_gpu=False)
...
"""
# 下面的语句传入两幅输入影像的路径
res = predictor.predict("./saved_files/cls/img.png")
print(res)
...
def label_to_RGB(image):
    ...
    index = image == 0
    RGB[index] = np.array([255, 0, 0]) #草地
    index = image == 1
    RGB[index] = np.array([30, 255, 142]) #林地
    index = image == 2
    RGB[index] = np.array([60, 0, 255]) #道路
    index = image == 3
    RGB[index] = np.array([255, 222, 0]) #其他
    index = image == 4
    RGB[index] = np.array([0, 0, 0]) #未标注
    return RGB
result = label_to_RGB(cm)
# 从左到右依次显示：第一时相影像、第二时相影像、整图推理结果以及真值标签
plt.figure(constrained_layout=True)
plt.subplot(121)
plt.imshow(Image.open("./saved_files/cls/img.png"))
...
plt.gca().set_axis_off()
plt.title("Pred")
cv2.imwrite("./saved_imgs/cls/test.png",result)
plt.savefig('./saved_imgs/cls/cls_res.png')
#plt.subplot(144)
